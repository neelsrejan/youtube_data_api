...
class Activities():
    ...
    def get_activities(self, num_vids):
        self.list_content_details(num_vids)
    ...
    def write_content_details(self, results, num_vids):
        with open(f"{self.channel_name}_content_details_{num_vids}_activities.json", "w") as f:
            json.dump(results, f, indent = 4)

    def write_id(self, results, num_vids):
        with open(f"{self.channel_name}_id_{num_vids}_activities.json", "w") as f:
            json.dump(results, f, indent = 4)

    def write_snippet(self, results, num_vids):
        with open(f"{self.channel_name}_snippet_{num_vids}_activities.json", "w") as f:
            json.dump(results, f, indent = 4)
